...
export const analysisService = {
     ...
     */
    transformBackendResponse(backendData: any): AnalysisResult {
        // similar_conditions를 similar_diseases 형식으로 변환
        let similarDiseases = [];
        if (backendData.similar_conditions) {
            const conditions = backendData.similar_conditions.split(', ');
            similarDiseases = conditions.map((condition: string, index: number) => ({
                name: condition.trim(),
                confidence: Math.max(10, (backendData.confidence_score || 0.5) * 100 - (index + 1) * 10), // 임시 계산
                description: `${condition.trim()}과 유사한 증상을 보일 수 있습니다.`
            }));
        }

        return {
            id: backendData.id,
            predicted_disease: backendData.diagnosis,
            confidence: Math.round((backendData.confidence_score || 0.5) * 100),
            summary: backendData.recommendations || '진단 소견이 제공되지 않았습니다.',
            recommendation: backendData.recommendations || '전문의 상담을 권장합니다.',
            similar_diseases: similarDiseases,
            metadata: {
                processing_time_seconds: backendData.metadata?.processing_time_seconds || 0,
                image_info: backendData.metadata?.image_info,
                image_size_kb: backendData.metadata?.image_size_kb,
